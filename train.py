...
def train(opt):
    ...
    def save_checkpoint(model, infos, optimizer, histories=None, append=''):
        ...
        with open(os.path.join(opt.checkpoint_path, 'infos'+opt.id+'%s.pkl' %(append)), 'wb') as f:
            utils.pickle_dump(infos, f)
        if histories:
            with open(os.path.join(opt.checkpoint_path, 'histories'+opt.id+'%s.pkl' %(append)), 'wb') as f:
    ...
    try:
        while True:
            ...
            if (iteration % opt.save_checkpoint_every == 0):
                ...
                print('> Writer Validation Summary')
                add_summary_value(tb_summary_writer, 'validation loss', val_loss, iteration)
                if lang_stats is not None:
                    for k,v in lang_stats.items():
                        add_summary_value(tb_summary_writer, k, v, iteration)
                val_result_history[iteration] = {'loss': val_loss, 'lang_stats': lang_stats, 'predictions': predictions}
                print('> Finish Write Validation')
            ...
            print('Debug Die Here End')
            if epoch >= opt.max_epochs and opt.max_epochs != -1:
                save_checkpoint(model, infos, optimizer, append='L ast Epochs')
                print('Reach Limit Epochs')
